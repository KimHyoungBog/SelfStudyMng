...
## 완료 조건 (관찰 가능한 동작)
- [ ] `/settings`에서 과목 추가 후 새로고침해도 목록이 유지된다
- [ ] `/`에서 오늘 요일에 등록된 과목만 표시된다
- [ ] 완료 체크 + 실제 시간 입력 후 새로고침해도 값이 유지된다
- [ ] `/weekly`에서 이번 주 각 과목의 목표 합계와 실제 합계가 정확히 표시된다
- [ ] 과목 미등록 상태에서 `/` 접속 시 설정 화면 링크가 표시된다
- [ ] 768px 미만 화면에서 레이아웃이 깨지지 않는다
