...
**Phase 6 — 주간 이력 조회 (2026-07-30 추가)**
- [x] T18: `getWeekDates`에 주 오프셋 파라미터 추가, "이전 주 / 다음 주 / 이번 주로" 네비게이션 UI
- [x] T19: 현재 주 초과 미래 이동 방지 (다음 주 버튼 비활성화)
- [x] T20: 과거 주에 기록이 없을 때 안내 문구 표시
- [x] T21: Playwright E2E TC5 — 이전 주 이동 시 과거 기록 표시, 이번 주 복귀 검증
...
- [x] `/weekly`에서 "이전 주" 클릭 시 지난 주 날짜 범위와 그 주의 기록이 표시된다
- [x] `/weekly`에서 현재 주일 때 "다음 주" 버튼이 비활성화된다
