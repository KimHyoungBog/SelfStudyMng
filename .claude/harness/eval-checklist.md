...
## Phase 6 평가 — 주간 이력 조회 기능 (2026-07-30)

### Stage 1 — Spec Compliance
- [x] `npm run test:e2e` 3/3 통과 (TC1, TC4, TC5 신규)
- [x] `npx tsc --noEmit` 0 에러
- [x] `npm run lint` 0 에러
- [x] "이전 주" 클릭 시 지난 주 날짜 범위·기록 표시 — 10주 연속 이동(월 경계), 연도 경계(2026-01-05 기준 clock 고정) 모두 정상 확인
- [x] 현재 주에서 "다음 주" 버튼 비활성화 — DOM 우회 시나리오까지 이중 방어 확인

### 결함 보고서
- 결함 #1 [major] — 과거 주 목표 시간이 과목의 **현재** 설정을 소급 반영(시점 스냅샷 없음) → `spec.md` "명시된 가정"에 의도적 단순화로 명문화. 코드 수정 보류(데이터 모델 확장 필요, 이번 스코프 밖)
- 결함 #2 [minor] — 과목 삭제 시 관련 과거 기록도 함께 삭제됨(기존 동작) → `spec.md`에 가정으로 명문화, 유지
- 결함 #3 [minor] — 과목 0개일 때 `/weekly` 주간 네비게이션 자체가 렌더링 안 됨(기존 early return) → 결함 #2로 인해 실질 영향 적음, 조치 보류
- 결함 #4 — 새로고침 시 조회 중이던 주(weekOffset) 리셋됨 — spec 요구사항 없어 결함 아님, 참고 기록
- 결함 #5 — 모바일 375px 레이아웃 정상, 결함 아님
